...
/**
 * 投票者一覧表期日前表帳票出力のキーブレイクの処理を行うクラスです。
 *
 * @reamsid_L AF-0270-020 xul
 */
public class TohyoshaListSenkyoKijitsumaeBreakEditor {

    private final TohyoshaListSenkyoAllParam 投票者一覧表期日前表;
     ...
     */
    public TohyoshaListSenkyoKijitsumaeBreakEditor(AFABTE503SelectProcessParameter parameter) {
        改頁リスト = parameter.get改頁項目();
        TohyoJokyoEntity 投票状況 = new TohyoJokyoEntity();
        GamenInfo 画面入力項目 = new GamenInfo();
        画面入力項目.set投票事由(parameter.get投票事由コード());
        画面入力項目.set期日前投票日開始(toRDate(parameter.get期日前投票日Fr()));
        画面入力項目.set期日前投票日終了(toRDate(parameter.get期日前投票日To()));
        画面入力項目.set投票時刻開始(toRString(parameter.get投票時刻Fr()));
        画面入力項目.set投票時刻終了(toRString(parameter.get投票時刻To()));
        投票者一覧表期日前表 = new TohyoshaListSenkyoAllParam(投票状況, 画面入力項目);
        投票者一覧表期日前表.set改頁リスト(改頁リスト);
    }
    ...
    public void usualProcess(NippoPSMPrintEntity current, RString 投票区名, List<RString> 選挙略称List, RString 投票場所) {
        ...
        if (current.get投票状況Entity().getUketsukeNo() == null) {
            投票者一覧表期日前表.get投票状況Entity().set受付番号(RString.EMPTY);
        } else {
            投票者一覧表期日前表.get投票状況Entity().set受付番号(toRString(current.get投票状況Entity().getUketsukeNo()));
        }
        投票者一覧表期日前表.get投票状況Entity().set投票区(current.get選挙人名簿Entity().getTohyokuCode());
        投票者一覧表期日前表.get投票状況Entity().set頁(current.get選挙人名簿Entity().getPage());
        投票者一覧表期日前表.get投票状況Entity().set行(current.get選挙人名簿Entity().getGyo());
        投票者一覧表期日前表.get投票状況Entity().set名称(toRString(current.get宛名PSMEntity().getMeisho()));
        投票者一覧表期日前表.get投票状況Entity().set性別コード(current.get宛名PSMEntity().getSeibetsuCode());
        投票者一覧表期日前表.get投票状況Entity().set生年月日(toRDate(current.get宛名PSMEntity().getSeinengappiYMD()));
        投票者一覧表期日前表.get投票状況Entity().set住民種別(current.get宛名PSMEntity().getJuminShubetsuCode());
        投票者一覧表期日前表.get投票状況Entity().set投票事由コード(toRString(current.get投票状況Entity().getTohyoJiyuCode()));
        投票者一覧表期日前表.get投票状況Entity().set請求方法(toRString(current.get投票状況Entity().getSeikyuHoho()));
        投票者一覧表期日前表.get投票状況Entity().set施設コード(current.get投票状況Entity().getShisetsuCode());
        投票者一覧表期日前表.get投票状況Entity().set投票状況(toRString(current.get投票状況Entity().getTohyoJokyo()));
        投票者一覧表期日前表.get投票状況Entity().set交付年月日(toRDate(current.get投票状況Entity().getKofuYMD()));
        投票者一覧表期日前表.get投票状況Entity().set受理年月日(toRDate(current.get投票状況Entity().getJuriYMD()));
        投票者一覧表期日前表.get投票状況Entity().set投票年月日(toRDate(current.get抄本Entity().getTohyoYMD()));
        投票者一覧表期日前表.get投票状況Entity().set交付方法(toRString(current.get投票状況Entity().getKofuHoho()));
        投票者一覧表期日前表.get投票状況Entity().set代理投票有フラグ(current.get投票状況Entity().getDairiTohyoAriFlag());
        投票者一覧表期日前表.get投票状況Entity().set点字フラグ(current.get投票状況Entity().getTenjiFlag());
        投票者一覧表期日前表.get投票状況Entity().set期日前不在者_場合(new RString("1"));
        投票者一覧表期日前表.set投票場所(投票場所);
        投票者一覧表期日前表.set選挙略称List(選挙略称List);
        ...
        投票者一覧表期日前表.get投票状況Entity().set識別コード(shikibeisuCode.value());
        this.投票区名 = 投票区名;
        投票計数(current);
    }
