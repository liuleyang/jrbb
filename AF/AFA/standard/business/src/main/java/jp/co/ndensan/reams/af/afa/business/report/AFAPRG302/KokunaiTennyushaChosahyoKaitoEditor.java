...
/**
 * 国内転入者調査票（回答）のEditorクラスです。
 *
 * @reamsid_L AF-0360-067 b_caiyf
 */
public class KokunaiTennyushaChosahyoKaitoEditor implements IKokunaiTennyushaChosahyoKaitoEditor {
     ...
     */
    public KokunaiTennyushaChosahyoKaitoEditor(KokunaiTennyushaChosahyoKaitoParameter target) {
        this.target = target;
        宛名識別対象 = ShikibetsuTaishoFactory.createShikibetsuTaisho(target.get選挙人資格の情報().getShikibetsuTaishoEntity());
    }

    @Override
    public KokunaiTennyushaChosahyoKaitoSource edit(KokunaiTennyushaChosahyoKaitoSource source) {
        setヘッダ(source);
        set明細タイトル(source);
        set明細(source);
        setフッタ(source);
    ...
    private void setヘッダ(KokunaiTennyushaChosahyoKaitoSource source) {
    ...
    private void set明細タイトル(KokunaiTennyushaChosahyoKaitoSource source) {
        source.aiteCityKubun = get転入先市区町村区分名称();

    }

    private void set明細(KokunaiTennyushaChosahyoKaitoSource source) {
        set氏名(source);
    ...
    private void setフッタ(KokunaiTennyushaChosahyoKaitoSource source) {
        FlexibleDate nowDate = FlexibleDate.getNowDate();
        RString 固定文D1 = CodeMaster.getCodeMeisho(AFACodeShubetsu.固定文_国内転入者調査票_回答.getCodeShubetsu(),
                new Code(KoteibunCode.固定文コード_１１.getCode()), nowDate);
        RString 固定文D2 = CodeMaster.getCodeMeisho(AFACodeShubetsu.固定文_国内転入者調査票_回答.getCodeShubetsu(),
    ...
    private void set転入先市区町村名(KokunaiTennyushaChosahyoKaitoSource source) {
        if (target.get導入団体() != null) {
            RString 都道府県名 = target.get導入団体().get都道府県名() == null ? RString.EMPTY : target.get導入団体().get都道府県名();
            RString 市区町村名 = target.get導入団体().get市町村名() == null ? RString.EMPTY : target.get導入団体().get市町村名();
    ...
    private void set転入前市区町村名(KokunaiTennyushaChosahyoKaitoSource source) {
    ...
    private void set氏名(KokunaiTennyushaChosahyoKaitoSource source) {
    ...
    private void editログの出力(KokunaiTennyushaChosahyoKaitoSource source) {
