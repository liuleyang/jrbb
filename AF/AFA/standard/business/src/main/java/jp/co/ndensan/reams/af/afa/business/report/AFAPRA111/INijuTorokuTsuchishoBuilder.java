package jp.co.ndensan.reams.af.afa.business.report.AFAPRA111;
/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */

import jp.co.ndensan.reams.af.afa.entity.report.AFAPRA111.NijuTorokuTsuchishoCommonSource;
import jp.co.ndensan.reams.uz.uza.report.IReportBuilder;

/**
 * {@link NijuTorokuTsuchishoReportSource}をソースファイルへ出力できる形へ編集するインタフェースです。
 *
 */
public interface INijuTorokuTsuchishoBuilder extends IReportBuilder<NijuTorokuTsuchishoCommonSource> {

}
