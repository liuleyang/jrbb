...
/**
 * 投票者一覧表混在表帳票出力のキーブレイクの処理を行うクラスです。
 *
 * @reamsid_L AF-0270-020 xul
 */
public class TohyoshaListSenkyoAllBreakEditor {

    private final TohyoshaListSenkyoAllParam 投票者一覧表混在表;
     ...
     */
    public TohyoshaListSenkyoAllBreakEditor(AFABTE503SelectProcessParameter parameter) {
        ...
        投票者一覧表混在表 = new TohyoshaListSenkyoAllParam(投票状況, 画面入力項目);
        投票者一覧表混在表.set改頁リスト(改頁リスト);
    }
    ...
    public void usualProcess(NippoPSMPrintEntity current, RString 投票区名, List<RString> 選挙略称List, RString 投票場所) {
        ...
        if (current.get投票状況Entity().getUketsukeNo() == null) {
            投票者一覧表混在表.get投票状況Entity().set受付番号(RString.EMPTY);
        } else {
            投票者一覧表混在表.get投票状況Entity().set受付番号(toRString(current.get投票状況Entity().getUketsukeNo()));
        }
        投票者一覧表混在表.get投票状況Entity().set投票区(current.get選挙人名簿Entity().getTohyokuCode());
        投票者一覧表混在表.get投票状況Entity().set頁(current.get選挙人名簿Entity().getPage());
        投票者一覧表混在表.get投票状況Entity().set行(current.get選挙人名簿Entity().getGyo());
        投票者一覧表混在表.get投票状況Entity().set名称(toRString(current.get宛名PSMEntity().getMeisho()));
        投票者一覧表混在表.get投票状況Entity().set性別コード(current.get宛名PSMEntity().getSeibetsuCode());
        投票者一覧表混在表.get投票状況Entity().set生年月日(toRDate(current.get宛名PSMEntity().getSeinengappiYMD()));
        投票者一覧表混在表.get投票状況Entity().set住民種別(current.get宛名PSMEntity().getJuminShubetsuCode());
        投票者一覧表混在表.get投票状況Entity().set投票事由コード(toRString(current.get投票状況Entity().getTohyoJiyuCode()));
        投票者一覧表混在表.get投票状況Entity().set請求方法(toRString(current.get投票状況Entity().getSeikyuHoho()));
        投票者一覧表混在表.get投票状況Entity().set施設コード(current.get投票状況Entity().getShisetsuCode());
        投票者一覧表混在表.get投票状況Entity().set投票状況(toRString(current.get投票状況Entity().getTohyoJokyo()));
        投票者一覧表混在表.get投票状況Entity().set交付年月日(toRDate(current.get投票状況Entity().getKofuYMD()));
        投票者一覧表混在表.get投票状況Entity().set受理年月日(toRDate(current.get投票状況Entity().getJuriYMD()));
        投票者一覧表混在表.get投票状況Entity().set投票年月日(toRDate(current.get抄本Entity().getTohyoYMD()));
        投票者一覧表混在表.get投票状況Entity().set交付方法(toRString(current.get投票状況Entity().getKofuHoho()));
        投票者一覧表混在表.get投票状況Entity().set代理投票有フラグ(current.get投票状況Entity().getDairiTohyoAriFlag());
        投票者一覧表混在表.get投票状況Entity().set点字フラグ(current.get投票状況Entity().getTenjiFlag());
        投票者一覧表混在表.get投票状況Entity().set期日前不在者_場合(new RString("3"));
        投票者一覧表混在表.set投票場所(投票場所);
        投票者一覧表混在表.set選挙略称List(選挙略称List);
        ...
        投票者一覧表混在表.get投票状況Entity().set識別コード(shikibeisuCode.value());
        this.投票区名 = 投票区名;
        投票計数(current);
    }
