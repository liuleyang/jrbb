 ...
 */
public class TojitsuTohyorokuFirstEditor extends KijitsumaeTohyorokuCommon implements ITojitsuTohyorokuEditor {
     ...
     */
    public TojitsuTohyorokuFirstEditor(TohyorokuChohyo item) {
        super();
        this.item = item;
    }
    ...
    public TojitsuTohyorokuReportSource edit(TojitsuTohyorokuReportSource source) {
        set帳票情報(source);
        source.layout = TojitsuTohyorokuReportSource.Layouts.AFAPRE301_TojitsuTohyoroku1mai1;
        return source;
    }

    private void set帳票情報(TojitsuTohyorokuReportSource source) {
        if (this.isLastFlg()) {
            set投票基本情報(source);
            set選任リスト(source);
            set投票管理者リスト(source);
            ...
            set投票拒否者法3リスト(source);
        }
        set代理投票者リスト(source);

    }

    private void set投票基本情報(TojitsuTohyorokuReportSource source) {
        source.shikkoYmd = getKanjiYmd(item.get執行年月日());
        source.title = item.getタイトル().concat(タイトル_当日投票所投票録);
        source.tohyokuCode = item.get投票区コード();
        ...
        source.meiboTorokushaMan = getFormatNumber(item.get名簿登録者男数());
        source.meiboTorokushaWoman = getFormatNumber(item.get名簿登録者女数());
        source.meiboTorokushaKei = getFormatNumber(item.get名簿登録者合計());
        source.tohyoshaSosuMan = getFormatNumber(item.get投票者男数());
        source.kariTohyoshaSuMan = getFormatNumber(item.get仮投票者男数());
        source.tohyoshaSosuWoman = getFormatNumber(item.get投票者女数());
        source.kariTohyoshaSuWoman = getFormatNumber(item.get仮投票者女数());
        source.tohyoshaSosuKei = getFormatNumber(item.get投票者合計());
    ...
    private void set選任リスト(TojitsuTohyorokuReportSource source) {
    ...
    private void set投票管理者リスト(TojitsuTohyorokuReportSource source) {
    ...
    private void set再交付者リスト(TojitsuTohyorokuReportSource source) {
    ...
    private void set決定投票者リスト(TojitsuTohyorokuReportSource source) {
    ...
    private void set返還者リスト(TojitsuTohyorokuReportSource source) {
    ...
    private void set代理投票者リスト(TojitsuTohyorokuReportSource source) {
    ...
    private void set投票拒否者法1リスト(TojitsuTohyorokuReportSource source) {
    ...
    private void set投票拒否者法2リスト(TojitsuTohyorokuReportSource source) {
    ...
    private void set投票拒否者法3リスト(TojitsuTohyorokuReportSource source) {
        source.hoNo3 = item.get法3();
    }
