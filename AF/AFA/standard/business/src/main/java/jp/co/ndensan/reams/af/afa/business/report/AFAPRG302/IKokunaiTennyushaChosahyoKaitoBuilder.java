/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package jp.co.ndensan.reams.af.afa.business.report.AFAPRG302;

import jp.co.ndensan.reams.af.afa.entity.report.AFAPRG302.KokunaiTennyushaChosahyoKaitoSource;
import jp.co.ndensan.reams.uz.uza.report.IReportBuilder;

/**
 * {@link KokunaiTennyushaChosahyoKaitoSource} をソースファイルへ出力できる形へ編集するインタフェースです。
 *
 * @reamsid_L AF-0360-067 b_caiyf
 */
public interface IKokunaiTennyushaChosahyoKaitoBuilder extends IReportBuilder<KokunaiTennyushaChosahyoKaitoSource> {

}
