...
/**
 * 年代別投票率集計表の出力該当CSV対応作成クラスです。
 *
 * @reamsid_L AF-0290-035 lit
 */
public final class NendaibetsuTohyoritsuCsvEntityEditor {

    private final TohyoritsuData data;
    private static final int 計算用_除法の百 = 100;

    private NendaibetsuTohyoritsuCsvEntityEditor(TohyoritsuData data) {
        this.data = data;
    }
     ...
     * @return NendaibetsuTohyoritsuCsvEntityEditor
     */
    public static NendaibetsuTohyoritsuCsvEntityEditor createInstance(TohyoritsuData data) {
        return new NendaibetsuTohyoritsuCsvEntityEditor(data);
    }

    /**
     * 年代別投票率集計表CSVクラスを編集する。
     *
     * @return NendaibetsuTohyoritsuCsvEntity 年代別投票率集計表CSVクラス
     */
    public NendaibetsuTohyoritsuCsvEntity createEntity() {
        NendaibetsuTohyoritsuCsvEntity csvEntity = new NendaibetsuTohyoritsuCsvEntity();
        ...
        csvEntity.set年代(年代Data.getラベル());
        csvEntity.set有権者男(new RString(年代Data.get有権者男数().toString()));
        csvEntity.set有権者女(new RString(年代Data.get有権者女数().toString()));
        csvEntity.set有権者計(new RString(年代Data.get有権者数().toString()));
        csvEntity.set投票者男(new RString(年代Data.get投票者男数().toString()));
        csvEntity.set投票者女(new RString(年代Data.get投票者女数().toString()));
        csvEntity.set投票者計(new RString(年代Data.get投票者数().toString()));
        csvEntity.set投票率男(getFormatDivide(年代Data.get投票者男数(), 年代Data.get有権者男数()));
        csvEntity.set投票率女(getFormatDivide(年代Data.get投票者女数(), 年代Data.get有権者女数()));
        csvEntity.set投票率計(getFormatDivide(年代Data.get投票者数(), 年代Data.get有権者数()));
