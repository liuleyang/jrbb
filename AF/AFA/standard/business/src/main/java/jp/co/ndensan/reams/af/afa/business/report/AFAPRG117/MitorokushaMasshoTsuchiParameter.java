...
@Getter
@Setter
public class MitorokushaMasshoTsuchiParameter {
     ...
     */
    public MitorokushaMasshoTsuchiParameter(MitorokushaMasshoTsuchiListEntity 選挙人資格の情報,
            FlexibleDate 発行日, ShichosonAtesaki 他市町村送付先, RString 発行番号_第,
            RString 発行番号_番号, RString 発行番号_号, Association 導入団体,
            RString 全国住所都道府県名, RString 全国住所市区町村名) {
     ...
     */
    public RString get転入前全国住所コード上5桁(MitorokushaMasshoTsuchiListEntity entity) {
        RString 転入前全国住所コード = getNullToRString(entity.getShikibetsuTaishoEntity().getTennyumaeZenkokuJushoCode());
        if (転入前全国住所コード.isNullOrEmpty()) {
            return RString.EMPTY;
        } else {
            return 転入前全国住所コード.substring(0, CodeTopType.コード上5桁.value());
        }
    }
