...
/**
 * 期日前投票の事由に関する調書帳票出力のキーブレイクの処理を行うクラスです。
 *
 * @reamsid_L AF-0270-020 xul
 */
public class TohyoJiyuChoshoKijitsumaeBreakEditor {

    private final TohyoJiyuChoshoParam 期日前投票調書;
     ...
     */
    public TohyoJiyuChoshoKijitsumaeBreakEditor(AFABTE503SelectProcessParameter parameter) {
        明細計数 = new HashMap<>();
        putMap(明細計数);
        選挙明細計数 = new HashMap<>();
        putMap(選挙明細計数);
        TohyoJokyoEntity 投票状況 = new TohyoJokyoEntity();
        期日前投票調書 = new TohyoJiyuChoshoParam(投票状況, toRDate(parameter.get期日前投票日Fr()),
                toRDate(parameter.get期日前投票日To()), null, null);
    }
    ...
    public void usualProcess(NippoPrintEntity current) {
        lastData = current;
        期日前投票調書.get投票状況Entity().set投票状況(new RString("6"));
        期日前投票調書.get投票状況Entity().set選挙種類(current.get抄本Entity().getSenkyoShurui().value());
        期日前投票調書.get投票状況Entity().set投票区(current.get選挙人名簿Entity().getTohyokuCode());
        期日前投票調書.get投票状況Entity().set選挙番号(new RString(current.get投票状況Entity().getSenkyoNo().toString()));
        投票計数(current, 明細計数);
        投票計数(current, 選挙明細計数);
    }
    ...
    public TohyoJiyuChoshoParam keyBreakProcessBySenkyoNo1(NippoPrintEntity before, NippoPrintEntity current) {
        if (isNew選挙(before, current)) {
            期日前投票調書.get投票状況Entity().set投票区(計);
            調書セット(選挙明細計数);
            putMap(選挙明細計数);
            return 期日前投票調書;
        }
    ...
    private void 調書セット(Map<Code, List<Integer>> map) {
        期日前投票調書.set直接1(toZZ_ZZ9編集(map.get(コード1).get(0)));
        期日前投票調書.set郵送1(toZZ_ZZ9編集(map.get(コード1).get(1)));
        期日前投票調書.set計1(toZZ_ZZ9編集(map.get(コード1).get(0) + map.get(コード1).get(1)));
        期日前投票調書.set直接2(toZZ_ZZ9編集(map.get(コード2).get(0)));
        期日前投票調書.set郵送2(toZZ_ZZ9編集(map.get(コード2).get(1)));
        期日前投票調書.set計2(toZZ_ZZ9編集(map.get(コード2).get(0) + map.get(コード2).get(1)));
        期日前投票調書.set直接3(toZZ_ZZ9編集(map.get(コード3).get(0)));
        期日前投票調書.set郵送3(toZZ_ZZ9編集(map.get(コード3).get(1)));
        期日前投票調書.set計3(toZZ_ZZ9編集(map.get(コード3).get(0) + map.get(コード3).get(1)));
        期日前投票調書.set直接4(toZZ_ZZ9編集(map.get(コード4).get(0)));
        期日前投票調書.set郵送4(toZZ_ZZ9編集(map.get(コード4).get(1)));
        期日前投票調書.set計4(toZZ_ZZ9編集(map.get(コード4).get(0) + map.get(コード4).get(1)));
        期日前投票調書.set直接5(toZZ_ZZ9編集(map.get(コード5).get(0)));
        期日前投票調書.set郵送5(toZZ_ZZ9編集(map.get(コード5).get(1)));
        期日前投票調書.set計5(toZZ_ZZ9編集(map.get(コード5).get(0) + map.get(コード5).get(1)));
        期日前投票調書.set直接6(toZZ_ZZ9編集(map.get(コード6).get(0)));
        期日前投票調書.set郵送6(toZZ_ZZ9編集(map.get(コード6).get(1)));
        期日前投票調書.set計6(toZZ_ZZ9編集(map.get(コード6).get(0) + map.get(コード6).get(1)));
        期日前投票調書.set直接7(toZZ_ZZ9編集(map.get(コード7).get(0)));
        期日前投票調書.set郵送7(toZZ_ZZ9編集(map.get(コード7).get(1)));
        期日前投票調書.set計7(toZZ_ZZ9編集(map.get(コード7).get(0) + map.get(コード7).get(1)));
        期日前投票調書.set直接合計(toZZ_ZZ9編集(map.get(コード1).get(0)
                + map.get(コード2).get(0)
                + map.get(コード3).get(0)
                + map.get(コード4).get(0)
                + map.get(コード5).get(0)
                + map.get(コード6).get(0)
                + map.get(コード7).get(0)));
        期日前投票調書.set郵送合計(toZZ_ZZ9編集(map.get(コード1).get(1)
                + map.get(コード2).get(1)
                + map.get(コード3).get(1)
                + map.get(コード4).get(1)
                + map.get(コード5).get(1)
                + map.get(コード6).get(1)
                + map.get(コード7).get(1)));
        期日前投票調書.set計合計(toZZ_ZZ9編集(map.get(コード1).get(0)
                + map.get(コード2).get(0)
                + map.get(コード3).get(0)
                + map.get(コード4).get(0)
                + map.get(コード5).get(0)
