 ...
 */
public class HyojiShojoshaMeiboReportProcess extends BatchKeyBreakBase<TorokushaMeiboEntity> {
    ...
    protected void initialize() {
        ...
        builder = new ShinkiTorokushaMeiboParam.ShinkiTorokushaMeiboParamBuilder(ReportIdKeys.表示消除者名簿.value());
    ...
    protected IBatchReader createReader() {
        return new BatchDbReader(new RString(
                "jp.co.ndensan.reams.af.afa.persistence.db.mapper.teijitouroku.IAFABTA101Mapper.selectHyojiShojyoshaMeiboList"),
                mybatisparameter);
    }

    @Override
    protected void createWriter() {
        batchReportWriter = BatchReportFactory.createBatchReportWriter(ReportIdKeys.表示消除者名簿.value()).create();
    ...
    private ShinkiTorokushaMeiboParam get空白行() {
        return new ShinkiTorokushaMeiboParam(getBefore(), ReportIdKeys.表示消除者名簿.value(), RString.EMPTY,
