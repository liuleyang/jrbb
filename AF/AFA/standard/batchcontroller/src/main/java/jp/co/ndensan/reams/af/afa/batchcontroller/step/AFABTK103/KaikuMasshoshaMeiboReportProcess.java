 ...
 */
public class KaikuMasshoshaMeiboReportProcess extends BatchKeyBreakBase<SenkyojiKaikuEntity> {
    ...
    @BatchWriter
    private BatchReportWriter<ShinkiTorokushaMeiboKaikuSource> 抹消者名簿Writer;
    private ReportSourceWriter<ShinkiTorokushaMeiboKaikuSource> 抹消者名簿SourceWriter;
    ...
    protected void createWriter() {
        抹消者名簿Writer = BatchReportFactory.createBatchReportWriter(ReportIdKeys.抹消者名簿海区.value())
                .addBreak(new BreakerCatalog<ShinkiTorokushaMeiboKaikuSource>().simplePageBreaker(抹消者名簿_BREAK_KEYS)).create();
        抹消者名簿SourceWriter = new ReportSourceWriter<>(抹消者名簿Writer);
    }
    ...
    protected void keyBreakProcess(SenkyojiKaikuEntity entity) {
        ...
        if (hasBreak(getBefore(), entity) && is抹消者(entity) && isRight抹消事由(entity)) {
            ...
            空白行.writeBy(抹消者名簿SourceWriter);

            ShinkiTorokushaMeiboKaikuParam 集計行 = builder.addData(entity, get抹消事由コード(entity), true, key);
            ShinkiTorokushaMeiboKaikuReport 集計行Out = new ShinkiTorokushaMeiboKaikuReport(集計行, 帳票共通ヘッダー);
            集計行Out.writeBy(抹消者名簿SourceWriter);
        }
    ...
    protected void usualProcess(SenkyojiKaikuEntity entity) {
        if (is抹消者(entity) && isRight抹消事由(entity)) {
            ...
            report.writeBy(抹消者名簿SourceWriter);
        }
    ...
    protected void afterExecute() {
        if (getBefore() != null && is抹消者(getBefore()) && isRight抹消事由(getBefore())) {
            ...
            ShinkiTorokushaMeiboKaikuParam param = builder.addData(getBefore(), get抹消事由コード(getBefore()), true, key);
            ShinkiTorokushaMeiboKaikuReport report = new ShinkiTorokushaMeiboKaikuReport(param, 帳票共通ヘッダー);
            report.writeBy(抹消者名簿SourceWriter);
        }

        pageCount.setValue(抹消者名簿Writer.getPageCount());
    }
    ...
    private Boolean is抹消者(SenkyojiKaikuEntity entity) {
        ...
        FlexibleDate 年齢到達期限日 = FlexibleDate.EMPTY;
