...
import jp.co.ndensan.reams.af.afa.business.core.nippo.TohyoshaListSenkyoAllBreakEditor;
 ...
 */
public class TohyoshaListSenkyoAllReportProcess extends BatchKeyBreakBase<NippoPSMPrintEntity> {

    private static final RString MAPPER_ID
            = new RString("jp.co.ndensan.reams.af.afa.persistence.db.mapper.relate.nippo."
                    + "INippoMapper.select投票者一覧表全リスト");
    ...
    private TohyoshaListSenkyoAllBreakEditor editor;
    ...
    protected void beforeExecute() {
        editor = new TohyoshaListSenkyoAllBreakEditor(parameter);
    ...
    protected void createWriter() {
        if (null == pageBreakKeys || pageBreakKeys.isEmpty()) {
            batchReportWriter = BatchReportFactory.createBatchReportWriter(ReportIdKeys.AFAPRE503_投票者一覧表_混在.value()).
                    create();
        } else {
            batchReportWriter = BatchReportFactory.createBatchReportWriter(ReportIdKeys.AFAPRE503_投票者一覧表_混在.value()).
    ...
    protected void usualProcess(NippoPSMPrintEntity current) {
        ...
        TohyoshaListSenkyoResult result = nippoManager.get投票者一覧表混在セット(mybatisparameter);
