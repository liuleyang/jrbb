...
/**
 * 補正登録者名簿（縦覧）のプロセスクラスです。
 *
 * @reamsid_L AF-0360-052 qiuxy
 */
public class ShinkiTorokushaMeiboJuranKokuminProcess extends BatchKeyBreakBase<TorokushaMeiboEntity> {
    ...
    private RString 登録年月日検索基準国民投票用;
    ...
    protected void initialize() {
        ...
        帳票共通ヘッダー.set帳票ID(ReportIdKeys.新規登録者名簿縦覧国民.value());
        取扱注意者の出力方式 = BusinessConfig.get(
                ToriatsukaiChuishaShutsurokuHoshikiConfig.取扱注意者_出力方式, RDate.getNowDate(), SubGyomuCode.AFA選挙本体);
        登録年月日検索基準国民投票用 = BusinessConfig.get(
                ConfigKeysAFA.登録年月日検索基準, RDate.getNowDate(), SubGyomuCode.AFA選挙本体);
        男性の計 = Decimal.ZERO;
        女性の計 = Decimal.ZERO;
    }
    ...
    protected IBatchReader createReader() {
        return new BatchDbReader(new RString(
                "jp.co.ndensan.reams.af.afa.persistence.db.mapper.kokumintohyoji.IAFABTG101Mapper.selectHoseiTorokushaMeibo"),
                mybatisParameter);
    }
    ...
    protected void keyBreakProcess(TorokushaMeiboEntity entity) {
        if (getBefore() != null && hasBreak(getBefore(), entity)) {
            空白行出力();
            集計行出力(getBefore().getSenkyoShikakuEntity().getTorokuJiyuCode());
        }
    }
    ...
    private void 空白行出力() {
        ShinkiTorokushaMeiboJuranParameter param = new ShinkiTorokushaMeiboJuranParameter(getBefore(), 出力区分_空白行, 帳票共通ヘッダー, RString.EMPTY);
    ...
    private void set取扱い注意者判断(TorokushaMeiboEntity entity) {
        ...
        if (findeCautionShaMapperEntity != null && findeCautionShaMapperEntity.isGaitoKubun()) {
            ...
            ShinkiTorokushaMeiboJuranParameter param = new ShinkiTorokushaMeiboJuranParameter(entity,
                    取扱注意者の出力方式, 帳票共通ヘッダー, 登録年月日検索基準国民投票用);
            ShinkiTorokushaMeiboJuranReport report = new ShinkiTorokushaMeiboJuranReport(param);
            report.writeBy(reportSourceWriter);
        } else {
            set集計行(entity);
            ShinkiTorokushaMeiboJuranParameter param = new ShinkiTorokushaMeiboJuranParameter(entity,
                    出力区分_データ, 帳票共通ヘッダー, 登録年月日検索基準国民投票用);
            ShinkiTorokushaMeiboJuranReport report = new ShinkiTorokushaMeiboJuranReport(param);
            report.writeBy(reportSourceWriter);
        }
    }
    ...
    private void 集計行出力(Code 事由コード) {
        RString 事由Code;
        if (null == 事由コード) {
            事由Code = RString.EMPTY;
        } else {
            事由Code = 事由コード.value();
        }
        RString 事由 = RString.EMPTY;
        if (TorokuJiyuCodeKokumin.一号資格者.getCode().value().equals(事由Code)) {
            事由 = new RString("** 1号資格者 **［男");
        } else if (TorokuJiyuCodeKokumin.二号資格者.getCode().value().equals(事由Code)) {
            事由 = new RString("** 2号資格者 **［男");
        } else if (TorokuJiyuCodeKokumin.その他.getCode().value().equals(事由Code)) {
            事由 = new RString("** その他 **［男");
        }
        ShinkiTorokushaMeiboJuranParameter 集計行 = new ShinkiTorokushaMeiboJuranParameter(getBefore(),
                出力区分_集計行, 帳票共通ヘッダー, 登録年月日検索基準国民投票用);

        RStringBuilder 集計 = new RStringBuilder();
        集計.append(事由).append(DecimalFormatter.toコンマ区切りRString(男性の計, 0))
    ...
    private void create文字切れ一覧表に出力Writer() {
        CheckListLineItemSet pairs = CheckListLineItemSet.of(
                ShinkiTorokushaMeiboJuranKokuminProcess.特定項目.class,
                ShinkiTorokushaMeiboJuranKokuminProcess.チェック項目.class);
        ...
        batchReportWriter
                ...
                .reportId(ReportIdKeys.新規登録者名簿縦覧国民.value())
