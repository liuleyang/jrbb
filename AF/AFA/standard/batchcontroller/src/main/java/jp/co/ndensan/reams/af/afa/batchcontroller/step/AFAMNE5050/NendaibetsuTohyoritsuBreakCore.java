...
import jp.co.ndensan.reams.af.afa.business.euc.afapre520.NendaibetsuTohyoritsuCsvEntityEditor;
 ...
 */
public class NendaibetsuTohyoritsuBreakCore {
    ...
    private static final RString 文字列_当日分 = new RString("当日分");
    private static final RString 文字列_期日前_不在者分 = new RString("期日前＋不在者分");
     ...
     */
    public NendaibetsuTohyoritsuBreakCore(AFABTE505SelectProcessParameter parameter) {
        this.帳票共通ヘッダー = new ZenjitsuReportHeaderEditor().create帳票共通ヘッダー();
        this.parameter = parameter;

        ラベルリスト = new TohyoKekkaShukeihyo().get年齢年代ラベル(TohyoKekkaShukeihyo.LabelShurui.年代, parameter.getOrder());
    ...
    public void usualProcess(TohyokekkaPrintEntity current) {
        if (null != current) {
            countUp(getラベル(current), current);
    ...
    private RString getラベル(TohyokekkaPrintEntity current) {
        ...
        for (TohyoKekkaShukeihyo.NendaiShurui 年代種類 : TohyoKekkaShukeihyo.NendaiShurui.values()) {
            if (年代種類.getFrom年齢() <= 年齢 && 年齢 <= 年代種類.getTo年齢()) {
                ラベル = 年代種類.get表示内容();
                break;
            }
        }

        if (ラベルリスト.contains(ラベル)) {
            return ラベル;
        }
     ...
     * @return entity NendaibetsuTohyoritsuCsvEntity
     */
    public NendaibetsuTohyoritsuCsvEntity createCsvEntity(TohyoritsuData data) {
        NendaibetsuTohyoritsuCsvEntityEditor editor = NendaibetsuTohyoritsuCsvEntityEditor.createInstance(data);
