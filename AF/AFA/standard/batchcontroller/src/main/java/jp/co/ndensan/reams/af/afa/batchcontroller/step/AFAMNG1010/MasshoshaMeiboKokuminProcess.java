 ...
 */
public class MasshoshaMeiboKokuminProcess extends BatchKeyBreakBase<TorokushaMeiboEntity> {
    ...
    protected void usualProcess(TorokushaMeiboEntity entity) {
        if (entity != null) {
            ...
            if (this.lastEntity != null && !this.lastEntity.getSenkyoninMeiboEntity()
                    .getTohyokuCode().equals(entity.getSenkyoninMeiboEntity().getTohyokuCode())
                    && !hasBreak(this.lastEntity, entity)) {
                空白行出力();
            }
            entity.set帳票ID(帳票ID);
            entity.setタイトル(抹消者名簿_タイトル);
            Code 抹消事由コード = entity.getSenkyoShikakuEntity().getMasshoJiyuCode();
            entity.setサブタイトル(CodeMaster.getCodeRyakusho(SubGyomuCode.AFA選挙本体,
                    AFACodeShubetsu.抹消事由コード_国民.getCodeShubetsu(), 抹消事由コード, 基準日));
            ...
            entity.setSenkyoEntity(senkyoEntity);
    ...
    private void 空白行出力() {
        lastEntity.set出力区分_空白行(Boolean.TRUE);
        ShinkiTorokushaMeiboKokuminReport report = new ShinkiTorokushaMeiboKokuminReport(lastEntity, 帳票共通ヘッダー);
        report.writeBy(reportSouceWriter);
    }

    private void 合計行出力() {

        lastEntity.set出力区分_合計行(Boolean.TRUE);
        Code masshoJiyuCode = lastEntity.getSenkyoShikakuEntity().getMasshoJiyuCode();
        if (masshoJiyuCode != null) {
            lastEntity.set合計(syukeiJyouhou.toRString(masshoJiyuCode.getColumnValue()));
            ShinkiTorokushaMeiboKokuminReport report = new ShinkiTorokushaMeiboKokuminReport(lastEntity, 帳票共通ヘッダー);
            report.writeBy(reportSouceWriter);
