...
import jp.co.ndensan.reams.af.afa.business.report.AFAPRG303.KokugaiTennyushaChosahyoKaitoPageBreak;
import jp.co.ndensan.reams.af.afa.business.report.AFAPRG303.KokugaiTennyushaChosahyoKaitoParameter;
import jp.co.ndensan.reams.af.afa.business.report.AFAPRG303.KokugaiTennyushaChosahyoKaitoReport;
...
/**
 * 国外転入者調査票（回答）のバッチ処理クラスです。
 *
 * @reamsid_L AF-0360-068 b_caiyf
 */
public class KokugaiTennyushaChosahyoKaitoProcess extends BatchProcessBase<MitorokushaMasshoTsuchiListEntity> {

    private KokuminTohyojiMybatisParameter mybatisparameter;
    private AFABTG101SelectProcessParameter parameter;
    @BatchWriter
    private IBatchReportWriterWithCheckList<KokugaiTennyushaChosahyoKaitoSource> btchReportWriter;
    private ReportSourceWriter<KokugaiTennyushaChosahyoKaitoSource> reportSouceWriter;
    ...
    protected void initialize() {
        ...
        発行番号_第 = parameter.get発行番号の第_国外転入者調査票();
        発行番号_番号 = parameter.get発行番号の番号_国外転入者調査票();
        発行番号_号 = parameter.get発行番号の号_国外転入者調査票();
        発行日 = parameter.get発行日_国外転入者調査票();
    ...
    protected void process(MitorokushaMasshoTsuchiListEntity entity) {
        set共通項目(entity);
        KokugaiTennyushaChosahyoKaitoParameter para = new KokugaiTennyushaChosahyoKaitoParameter(entity,
                発行日, 他市町村送付先, 発行番号_第, 発行番号_番号, 発行番号_号, 地方公共団体, 全国住所都道府県名,
                全国住所市区町村名);
        KokugaiTennyushaChosahyoKaitoReport report = new KokugaiTennyushaChosahyoKaitoReport(para);
    ...
    protected IBatchReader createReader() {
        return new BatchDbReader(new RString(
                "jp.co.ndensan.reams.af.afa.persistence.db.mapper.kokumintohyoji.IAFABTG101Mapper.selectKokugaiTennyushaChosahyoList"
        ), mybatisparameter);
    }
    ...
    private void create文字切れ一覧表に出力Writer() {
        CheckListLineItemSet pairs = CheckListLineItemSet.of(
                KokugaiTennyushaChosahyoKaitoProcess.特定項目.class,
                KokugaiTennyushaChosahyoKaitoProcess.チェック項目.class);
        ...
        btchReportWriter = BatchWriters
                .batchReportWriterWithCheckList(KokugaiTennyushaChosahyoKaitoSource.class)
                .checkListInfo(info)
                .checkListLineItemSet(pairs)
                .reportId(ReportIdKeys.国外転入者調査票回答.value())
                .addBreak(new KokugaiTennyushaChosahyoKaitoPageBreak())
                .build();
    }
