 ...
 */
package jp.co.ndensan.reams.af.afa.batchcontroller.flow.AFAMNG1050;
...
import jp.co.ndensan.reams.af.afa.batchcontroller.step.AFAMNG1050.MeiboSaishinkaKokuminUpdateProcess;
 ...
 */
public class AFABTG105_MeiboSaishinkaKokuminFlow extends BatchFlowBase<AFABTG105BatchParameter> {

    private static final String 投票人名簿抄本_出力 = "投票人名簿抄本出力";
    private static final String 投票人名簿登録者数リスト_出力 = "投票人名簿登録者数リスト出力";
    ...
    private static final String 一号資格登録通知_出力 = "1号資格登録通知出力";
    private static final String 一号資格登録通知一覧表_出力 = "1号資格登録通知一覧表出力";
    private static final String 二号資格調査票回答_出力 = "2号資格調査票回答出力";
    private static final String 二号資格調査票照会_出力 = "二号資格調査票照会出力";
    private static final String 二号資格調査一覧表_出力 = "2号資格調査一覧表出力";
    private static final String 二号資格候補者一覧表_出力 = "2号資格候補者一覧表出力";
    private static final String 補正登録者名簿_出力 = "補正登録者名簿出力";
    private static final String 補正登録者名簿縦覧用_出力 = "補正登録者名簿（縦覧用）出力";
    private static final String 抹消者名簿_出力 = "抹消者名簿出力";
    private static final String 抹消者名簿縦覧用_出力 = "抹消者名簿（縦覧用）出力";
    private static final String 訂正者名簿_出力 = "訂正者名簿出力";
    private static final String 未登録者抹消通知_出力 = "未登録者抹消通知出力";
    private static final String 未登録者抹消通知一覧_出力 = "未登録者抹消通知一覧出力";
    private static final String 国内転入者調査票照会_出力 = "国内転入者調査票照会出力";
    private static final String 国内転入者調査票回答_出力 = "国内転入者調査票回答出力";
    private static final String 国内転入者調査一覧表_出力 = "国内転入者調査一覧表出力";
    private static final String 国外転入者調査票照会_出力 = "国外転入者調査票照会出力";
    private static final String 国外転入者調査票回答_出力 = "国外転入者調査票回答出力";
    private static final String 国外転入者調査一覧表_出力 = "国外転入者調査一覧表出力";
    private static final String 在外投票人名簿抄本_出力 = "在外投票人名簿抄本出力";
    private static final String 在外投票資格者名簿_出力 = "在外投票資格者名簿出力";
    private static final String 在外投票管理表_出力 = "在外投票管理表出力";
    private static final String 投票人名簿最新化_データ更新 = "投票人名簿最新化データ更新";
    ...
    private AFABTG105SelectProcessParameter selectProcessParameter;
    private AFABTG101SelectProcessParameter aFABTG101SPParameter;
    ...
    protected void prepareConfigData() {
        ...
        build.set基準日(getParameter().get最新化時点日());
        shikibetsuTaishoPSMSearchKey = build.build();
        if (getParameter().get時点日() == null
                || getParameter().get時点日().isEmpty()) {
        ...
        selectProcessParameter = getParameter().toAFABTG105SelectProcessParameter(shikibetsuTaishoPSMSearchKey);
        aFABTG101SPParameter = getParameter().toAFABTG101SelectProcessParameter();
    ...
    @Step(投票人名簿最新化_データ更新)
    IBatchFlowCommand execute投票人名簿最新化データ更新() {
        return loopBatch(MeiboSaishinkaKokuminUpdateProcess.class).arguments(selectProcessParameter).define();
    }

    private void 投票人名簿抄本出力() {
        if (getParameter().get投票人名簿抄本出力有無()) {
            帳票ID = ReportIdKeys.投票人名簿抄本.value();
            aFABTG101SPParameter.setSort(getソート順WithMaster(帳票ID));
            executeStep(投票人名簿抄本_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit投票人名簿抄本出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 投票人名簿抄本_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
                    ReportIdKeys.投票人名簿抄本));
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 投票人名簿抄本_出力, ShutsuRyokuJyoukenEnum.ページコント縦覧.getValue()), 出力条件,
    ...
    IBatchFlowCommand creat投票人名簿抄本出力() {
        return loopBatch(TohyoninMeiboShohonReportProcess.class).arguments(aFABTG101SPParameter).define();
    }

    private void 投票人名簿登録者数リスト() {
        if (getParameter().get投票人名簿登録者数リスト出力有無()) {
            executeStep(投票人名簿登録者数リスト一時テーブル作成);
            executeStep(投票人名簿登録者数リスト_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit投票人名簿登録者数リスト出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 投票人名簿登録者数リスト_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    IBatchFlowCommand creat登録者数リスト一時テーブル() {
        return loopBatch(TorokushasuListDataSelectProcess.class).arguments(aFABTG101SPParameter).define();
    }

    @Step(投票人名簿登録者数リスト_出力)
    IBatchFlowCommand get登録者数リスト帳票出力() {
        aFABTG101SPParameter.setTable(getResult(RString.class,
                new RString(投票人名簿登録者数リスト一時テーブル作成), TorokushasuListDataSelectProcess.OUT_TEMP_TABLE_NAME));
        aFABTG101SPParameter.set選挙種類(getResult(SenkyoShurui.class,
                new RString(投票人名簿登録者数リスト一時テーブル作成), TorokushasuListDataSelectProcess.OUT_SENKYO_SHURUI));
        return loopBatch(TorokushasuListReportProcess.class).arguments(aFABTG101SPParameter).define();
    }

    private void 登録者数リスト不在者投票資格者分() {
        if (getParameter().get投票人名簿登録者数リスト出力有無()
            ...
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 登録者数リスト不在者分帳票出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    IBatchFlowCommand creat登録者数リスト不在者分一時テーブル() {
        return loopBatch(TorokushasuListFuzaiShikakuDataSelectProcess.class).arguments(aFABTG101SPParameter).define();
    }
    ...
    IBatchFlowCommand get登録者数リスト不在者分帳票出力() {
        ...
        return loopBatch(TorokushasuListFuzaiShikakuReportProcess.class).arguments(aFABTG101SPParameter).define();
    }
    ...
    private void 一号資格登録通知() {
        if (getParameter().get一号資格登録通知出力有無()) {
            executeStep(一号資格登録通知_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit一号資格登録通知出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 一号資格登録通知_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    IBatchFlowCommand createIchigoShikakuTorokuTsuchiReportProcess() {
        return loopBatch(IchigoShikakuTorokuTsuchiReportProcess.class).arguments(aFABTG101SPParameter).define();
    }

    private void 一号資格登録通知一覧表() {
        if (getParameter().get一号資格登録通知出力有無()
                && getParameter().get一号資格登録通知一覧表出力有無()) {
            executeStep(一号資格登録通知一覧表_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit一号資格登録通知一覧表出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 一号資格登録通知一覧表_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    IBatchFlowCommand get宛名シール1号資格登録通知12面() {
        return loopBatch(IchigoShikakuTorokuTsuchiAtenaSeal12Process.class).arguments(aFABTG101SPParameter).define();
    }

    @Step(宛名シール1号資格登録通知21面)
    IBatchFlowCommand get宛名シール1号資格登録通知21面() {
        return loopBatch(IchigoShikakuTorokuTsuchiAtenaSeal21Process.class).arguments(aFABTG101SPParameter).define();
    }

    private void 二号資格調査票照会() {
        if (getParameter().get二号資格調査票出力有無()) {
            executeStep(二号資格調査票照会_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit二号資格調査票出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 二号資格調査票照会_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    private void 二号資格調査票回答() {
        if (getParameter().get二号資格調査票出力有無()) {
            executeStep(二号資格調査票回答_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit二号資格調査票出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 二号資格調査票回答_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    private void 二号資格調査一覧表() {
        if (getParameter().get二号資格調査票出力有無()
            ...
            executeStep(二号資格調査一覧表_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit二号資格調査一覧表出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 二号資格調査一覧表_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    private void 二号資格候補者一覧表() {
        if (getParameter().get二号資格候補者一覧表出力有無()) {
            executeStep(二号資格候補者一覧表_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit二号資格候補者一覧表出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 二号資格候補者一覧表_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    private void 補正登録者名簿() {
        if (getParameter().get補正登録者名簿出力有無()) {
            executeStep(補正登録者名簿_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit補正登録者名簿出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 補正登録者名簿_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    private void 補正登録者名簿縦覧() {
        if (getParameter().get補正登録者名簿出力有無()
            ...
            executeStep(補正登録者名簿縦覧用_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit補正登録者名簿縦覧用出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 補正登録者名簿縦覧用_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    private void 抹消者名簿() {
        if (getParameter().get抹消者名簿出力有無()) {
            executeStep(抹消者名簿_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit抹消者名簿出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 抹消者名簿_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    private void 抹消者名簿縦覧() {
        if (getParameter().get抹消者名簿出力有無()
            ...
            executeStep(抹消者名簿縦覧用_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit抹消者名簿縦覧用出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 抹消者名簿縦覧用_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    private void 訂正者名簿() {
        if (getParameter().get訂正者名簿出力有無()) {
            executeStep(訂正者名簿_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit訂正者名簿出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 訂正者名簿_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    private void 未登録者抹消通知() {
        if (getParameter().get未登録者抹消通知出力有無()) {
            executeStep(未登録者抹消通知_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit未登録者抹消通知出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 未登録者抹消通知_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    private void 未登録者抹消通知一覧表() {
        if (getParameter().get未登録者抹消通知出力有無()
            ...
            executeStep(未登録者抹消通知一覧_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit未登録者抹消通知一覧出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 未登録者抹消通知一覧_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    private void 国内転入者調査票照会() {
        if (getParameter().get国内転入者調査票出力有無()) {
            executeStep(国内転入者調査票照会_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit国内転入者調査票出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 国内転入者調査票照会_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    private void 国内転入者調査票回答() {
        if (getParameter().get国内転入者調査票出力有無()) {
            executeStep(国内転入者調査票回答_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit国内転入者調査票出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 国内転入者調査票回答_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    private void 国内転入者調査一覧表() {
        if (getParameter().get国内転入者調査票出力有無()
            ...
            executeStep(国内転入者調査一覧表_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit国内転入者調査一覧表出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 国内転入者調査一覧表_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    private void 国外転入者調査票照会() {
        if (getParameter().get国外転入者調査票出力有無()) {
            executeStep(国外転入者調査票照会_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit国外転入者調査票出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 国外転入者調査票照会_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    private void 国外転入者調査票回答() {
        if (getParameter().get国外転入者調査票出力有無()) {
            executeStep(国外転入者調査票回答_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit国外転入者調査票出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 国外転入者調査票回答_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    private void 国外転入者調査一覧表() {
        if (getParameter().get国外転入者調査票出力有無()
            ...
            executeStep(国外転入者調査一覧表_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit国外転入者調査一覧表出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 国外転入者調査一覧表_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    private void 在外投票人名簿抄本() {
        if (getParameter().get在外投票人名簿抄本出力有無()) {
            ...
            executeStep(在外投票人名簿抄本_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit在外投票人名簿抄本出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 在外投票人名簿抄本_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    private void 在外投票資格者名簿() {
        if (getParameter().get在外投票資格者名簿出力有無()) {
            executeStep(在外投票資格者名簿_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit在外投票資格者名簿出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 在外投票資格者名簿_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    private void 在外投票管理表出力() {
        if (getParameter().get在外投票管理表出力有無()) {
            ...
            aFABTG101SPParameter.setSort(get在外ソート順(帳票ID));
            executeStep(在外投票管理表_出力);
            List<RString> 出力条件 = new ArrayList<>();
            出力条件 = edit在外投票管理表出力条件(出力条件);
            reportOutputJokenhyoItems.add(getReportOutputJokenhyoItem(
                    getResult(Integer.class, 在外投票管理表_出力, ShutsuRyokuJyoukenEnum.ページコント.getValue()), 出力条件,
    ...
    private ReportOutputJokenhyoItem getReportOutputJokenhyoItem(int 出力ページ数,
        ...
        List<RString> 出力条件リスト = edit投票人名簿最新化出力条件();
        出力条件リスト.addAll(出力条件);
    ...
    private List<RString> edit投票人名簿最新化出力条件() {
        ...
        出力条件リスト.add(ShutsuRyokuJyoukenEnum.出力条件_最新化時点日.getValue().concat(edit日時(getParameter().get最新化時点日())));
        出力条件リスト.add(ShutsuRyokuJyoukenEnum.出力条件_名簿並び替え.getValue().concat(
                getParameter().get名簿並び替え() != null
                && getParameter().get名簿並び替え() ? ShutsuRyokuJyoukenEnum.出力内容_あり.getValue() : ShutsuRyokuJyoukenEnum.出力内容_なし.getValue()));
        return 出力条件リスト;
    }
    ...
    private List<RString> edit一号資格登録通知出力条件(List<RString> 出力条件リスト) {
        出力条件リスト.add(ShutsuRyokuJyoukenEnum.出力条件_1号資格登録通知出力有無.getValue().concat(
                ...
                && getParameter().get一号資格登録通知出力有無() ? ShutsuRyokuJyoukenEnum.出力内容_ON.getValue() : ShutsuRyokuJyoukenEnum.出力内容_OFF.getValue()));
    ...
    private List<RString> edit二号資格調査票出力条件(List<RString> 出力条件リスト) {
        出力条件リスト.add(ShutsuRyokuJyoukenEnum.出力条件_2号資格調査票出力有無.getValue().concat(
                ...
                && getParameter().get二号資格調査票出力有無() ? ShutsuRyokuJyoukenEnum.出力内容_ON.getValue() : ShutsuRyokuJyoukenEnum.出力内容_OFF.getValue()));
