 ...
 */
public class ShinkiTorokushaMeiboJuranReportProcess extends BatchKeyBreakBase<TorokushaMeiboEntity> {
    ...
    protected IBatchReader createReader() {
        return new BatchDbReader(new RString(
                "jp.co.ndensan.reams.af.afa.persistence.db.mapper.teijitouroku.IAFABTA101Mapper.selectTorokusyaMeiboJuranboList"),
                mybatisparameter);
    }
    ...
    protected void keyBreakProcess(TorokushaMeiboEntity entity) {
        Code beforeJiyuCode = entity.getSenkyoShikakuEntity().getTorokuJiyuCode();
        if (beforeJiyuCode != null
                && !beforeJiyuCode.equals(getBefore().getSenkyoShikakuEntity().getTorokuJiyuCode())) {
            add空白行();
            add集計行(getBefore().getSenkyoShikakuEntity().getTorokuJiyuCode());
            男性の計 = Decimal.ZERO;
            女性の計 = Decimal.ZERO;
        }
    }
    ...
    private void add集計行(Code 事由コード) {
        RString 事由Code;
        if (null == 事由コード) {
        ...
        if (JiyuCode.年齢到達.getKey().equals(事由Code)) {
            事由 = new RString("** 年齢到達 **［男");
        } else if (JiyuCode.転入3カ月経過.getKey().equals(事由Code)) {
            事由 = new RString("** 転入3カ月経過 **［男");
        } else if (JiyuCode.その他_登録.getKey().equals(事由Code)) {
    ...
    private void set空白行(TorokushaMeiboEntity entity) {
        if (null == beforeEntity) {
            return;
        }
        if (!entity.getSenkyoninMeiboEntity().getTohyokuCode()
                .equals(beforeEntity.getSenkyoninMeiboEntity().getTohyokuCode())) {
            add空白行();
        }
    }
    ...
    private TeijiCommonJohoItem get帳票共通ヘッダー() {
        ...
        帳票共通ヘッダー.set帳票ID(ReportIdKeys.新規登録者名簿縦覧.value());
        return 帳票共通ヘッダー;
    }
    ...
    private void create文字切れ一覧表に出力Writer() {
        CheckListLineItemSet pairs = CheckListLineItemSet.of(
                ShinkiTorokushaMeiboJuranReportProcess.特定項目.class,
                ShinkiTorokushaMeiboJuranReportProcess.チェック項目.class);
        ...
        batchReportWriter
                ...
                .reportId(ReportIdKeys.新規登録者名簿縦覧.value())
