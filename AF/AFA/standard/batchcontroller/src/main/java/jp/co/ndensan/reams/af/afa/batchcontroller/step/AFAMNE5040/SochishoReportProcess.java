...
import jp.co.ndensan.reams.af.afa.business.report.AFAPRE519.SochishoReport;
 ...
 */
public class SochishoReportProcess extends BatchKeyBreakBase<ZenjitsuPrintEntity> {
    ...
    private static final List<RString> PAGE_BREAK_KEYS
            = Collections.unmodifiableList(Arrays.asList(new RString(SochishoReportSource.ReportSourceFileds.tohyokuCode.name()),
                            new RString(SochishoReportSource.ReportSourceFileds.senkyoNo.name())));

    private AFABTE504SelectProcessParameter parameter;
    private ZenjitsuPrintMybatisParameter mybatisparameter;

    @BatchWriter
    private BatchReportWriter<SochishoReportSource> batchReportWriter;
    private ReportSourceWriter<SochishoReportSource> reportSourceWriter;
    private SochishoBreakCore sochishoBreakCore;
    ...
    protected void beforeExecute() {
        sochishoBreakCore = new SochishoBreakCore(parameter);
    }
    ...
    protected void createWriter() {
        batchReportWriter = BatchReportFactory.createBatchReportWriter(ReportIdKeys.送致書.value()).
    ...
    protected void usualProcess(ZenjitsuPrintEntity current) {
        sochishoBreakCore.usualProcess(getBefore(), current);
    }

    @Override
    protected void keyBreakProcess(ZenjitsuPrintEntity current) {
        List<SochishoParam> paramList = sochishoBreakCore.keyBreakProcess(getBefore(), current);
        for (SochishoParam param : paramList) {
            SochishoReport report = new SochishoReport(getBefore(), param);
    ...
    protected void afterExecute() {
        if (null != getBefore()) {
            List<SochishoParam> paramList = sochishoBreakCore.keyBreakProcess(getBefore(), null);
            for (SochishoParam param : paramList) {
                SochishoReport report = new SochishoReport(getBefore(), param);
                report.writeBy(reportSourceWriter);
            }
