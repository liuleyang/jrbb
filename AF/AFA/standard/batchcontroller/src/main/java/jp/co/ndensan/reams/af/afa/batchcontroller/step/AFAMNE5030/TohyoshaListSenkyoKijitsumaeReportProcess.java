...
import jp.co.ndensan.reams.af.afa.business.core.nippo.TohyoshaListSenkyoKijitsumaeBreakEditor;
 ...
 */
public class TohyoshaListSenkyoKijitsumaeReportProcess extends BatchKeyBreakBase<NippoPSMPrintEntity> {

    private static final RString MAPPER_ID
            = new RString("jp.co.ndensan.reams.af.afa.persistence.db.mapper.relate.nippo."
                    + "INippoMapper.select投票者一覧期日前表");
    ...
    private TohyoshaListSenkyoKijitsumaeBreakEditor editor;
    ...
    protected void beforeExecute() {
        editor = new TohyoshaListSenkyoKijitsumaeBreakEditor(parameter);
    ...
    protected void createWriter() {
        if (null == pageBreakKeys || pageBreakKeys.isEmpty()) {
            batchReportWriter = BatchReportFactory.createBatchReportWriter(ReportIdKeys.AFAPRE503_投票者一覧表_期日前.value()).
                    create();
        } else {
            batchReportWriter = BatchReportFactory.createBatchReportWriter(ReportIdKeys.AFAPRE503_投票者一覧表_期日前.value()).
    ...
    protected void usualProcess(NippoPSMPrintEntity current) {
        ...
        TohyoshaListSenkyoResult result = nippoManager.get投票者一覧選挙期日前表セット(mybatisparameter);
